...
public class UserController {
     ...
     */
    @PostMapping("/login")
    public ObjectId login(@RequestBody @Validated LoginDTO validator, @NotNull BindingResult errors) {
        ...
        ObjectId id = userService.login(validator);
        log.info("用户登录请求: ==> 登录成功,用户 id 为: {}", id);
        return id;
    }
    ...
    public ObjectId register(@RequestBody @Validated RegisterDTO validator, @NotNull BindingResult errors) {
        ...
        ObjectId id = userService.register(validator);
        log.info("用户注册请求: ==> 注册成功,用户 id 为: {}", id);
        return id;
    }
}
